...
const Footer = () => {
  ...
  const quickLinks = [
    ...
    { name: t("header.sign_up_button"), icon: <UserPlus size={18} />, path: "/signup" },
    { name: t("header.login_button"), icon: <LogIn size={18} />, path: "/login" },
  ];
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div>
            ...
            <ul className="space-y-2">
              {quickLinks.map((item) => (
                <li key={item.name}>
                  <Link to={item.path} className="hover:text-indigo-400 transition-colors duration-300 flex items-center">
                    {item.icon}
          ...
          <div>
            ...
            <p>{t("footer.email")}: contact@videodataportal.com</p>
