        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature) => (
            <Link to={feature.path} key={feature.title}>
              <div
                className={`${feature.color} rounded-lg p-6 shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1 cursor-pointer`}
              >
        ...
        <section className="mt-16 bg-indigo-700 text-white rounded-lg p-8 shadow-lg">
          <div className="md:flex items-center justify-between">
            ...
            {!user ? (
              <Link
                to="/signup"
                className="bg-white text-indigo-700 px-6 py-2 rounded-full font-semibold hover:bg-indigo-100 transition-colors duration-300"
              >
                {t("sign_up_now")}
              </Link>
            ) : (
