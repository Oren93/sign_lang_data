...
const DataAccess = () => {
  ...
  const fetchGlosses = async () => {
    try {
      const response = await fetch('http://localhost:8001/glosses', {
        headers: {
            ...
            {filteredGlosses.map((gloss) => (
              <li
                ...
                className={`p-2 rounded-md cursor-pointer flex justify-between items-center ${
                  gloss.video_count > 0 ? 'bg-green-100 hover:bg-green-200' : 'bg-gray-100 hover:bg-gray-200'
                }`}
                onClick={() => setSelectedGloss(gloss)}
              >
                <span>{gloss.gloss}</span>
                <span className="text-sm text-gray-600">
                  {gloss.video_count} {gloss.video_count === 1 ? t('video') : t('videos')}
                </span>
              </li>
            ))}
          </ul>
