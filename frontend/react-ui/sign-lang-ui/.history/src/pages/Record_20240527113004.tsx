...
const Record: React.FC = () => {
  ...
  const generateRandomWord = () => {
    let availableWords = words.filter(word => !shownWords.includes(word));
    if (availableWords.length === 0) {
      setShownWords([]); // Reset shown words if all words have been shown
      availableWords = words;
    }
    const randomWord = availableWords[Math.floor(Math.random() * availableWords.length)];
    setWord(randomWord);
    setShownWords([...shownWords, randomWord]);
  };
